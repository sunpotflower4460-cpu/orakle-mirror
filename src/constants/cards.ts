...
// QRNG 主経路は非同期版 getRandomCardsQuantum（下記）に分離する。
export const getRandomCards = (num = 2): OracleCard[] => {
  ...
  const e = cryptoEntropy(recommendedByteLength(ORACLE_CARDS.length));
  return shuffleWithEntropy(ORACLE_CARDS, e).slice(0, clampedNum);
};
 ...
 */
export const getRandomCardsQuantum = async (
  num = 2,
): Promise<{ cards: OracleCard[]; source: EntropySource }> => {
  if (ORACLE_CARDS.length === 0) return { cards: [], source: 'crypto' };
  const clampedNum = Math.min(Math.max(0, num), ORACLE_CARDS.length);
  const byteLen = recommendedByteLength(ORACLE_CARDS.length);
  const e = await acquireEntropy(byteLen);
  try {
    return { cards: shuffleWithEntropy(ORACLE_CARDS, e).slice(0, clampedNum), source: e.source };
  } catch {
    // 万一エントロピーが尽きても必ず引けるよう crypto で引き直す
    const fallback = cryptoEntropy(byteLen * 2);
    return { cards: shuffleWithEntropy(ORACLE_CARDS, fallback).slice(0, clampedNum), source: 'crypto' };
  }
};
