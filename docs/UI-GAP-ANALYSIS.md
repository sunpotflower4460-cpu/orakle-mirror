...
| `backdrop-filter` 弱い環境のフォールバック | `src/MainApp.tsx`, `src/components/Onboarding.tsx` | 既存質感を維持した補助背景のみ追加 | P0 | 不要 |
| safe-area 差分 | `src/styles/globals.ts`, `src/MainApp.tsx`, `capacitor.config.ts` | 実機値を見て不足分のみ調整 | P0 | 不要 |
| 中央オーブの統一（Onboarding/Home/Sidebar） | `src/components/Onboarding.tsx`, `src/MainApp.tsx` | 既存表現を共通化、黒影を増やさない | P1 | 不要 |
